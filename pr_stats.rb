...
sample = records.select { |pr| pr.created > FOUR_MONTHS_AGO } #and (!pr.time_to_merge or pr.time_to_merge < 30) }
...
stats_all = DescriptiveStatistics::Stats.new(ttm_data(sample))
report(stats_all, 'All Merges (Last 4 months)')

stats_not_same_day = DescriptiveStatistics::Stats.new(ttm_data(sample, 1))
report(stats_not_same_day, 'PRs taking >0 days (Last 4 months)')

stats_three_days_plus = DescriptiveStatistics::Stats.new(ttm_data(sample, 3))
report(stats_three_days_plus, 'PRs taking >2 days (Last 4 months)')
